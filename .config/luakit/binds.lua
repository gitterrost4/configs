...
add_binds("normal", {
    ...
    key({}, "v", function (w) 
        ...
        if uri then
            ...
            print(string.format("urxvt -e cclive -f best --filename-format '%%t.%%s' "
                .. "--output-dir %q --exec='mplayer \"%%f\"' %q", os.getenv("HOME"), uri))
    ...
    key({}, "D", "Close all but current tab (or `[count]` tabs).",
        function (w, m) 
            t = w.tabs:current()
            for i=1,w.tabs:count() do if i~=t then w:close_tab() end end end),
...
add_cmds({
    ...
    cmd("nopriv[ate]", "blubb", function (w)
      w.view.enable_private_browsing = false
      w:notify("Non-private browsing.")
    end),
    cmd("priv[ate]", "blubb", function (w)
      w.view.enable_private_browsing = true
      w:notify("Private browsing.")
    end),
})
